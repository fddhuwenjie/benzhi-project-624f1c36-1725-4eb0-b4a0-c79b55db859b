...
var verifiedManifestCache sync.Map
...
func VerifyManifest(m DeliveryManifest) error {
	cacheKey := m.ManifestID + ":" + m.MasterVersion + ":" + m.ManifestSHA256
	if _, ok := verifiedManifestCache.Load(cacheKey); ok {
		return nil
	}
	...
	verifiedManifestCache.Store(cacheKey, struct{}{})
	return nil
}
