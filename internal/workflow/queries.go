...
func (s *Service) Readiness(ctx context.Context, caseID string) (domain.ReadinessReport, error) {
	...
	s.readinessMu.RLock()
	cached, ok := s.readinessCache[caseID]
	s.readinessMu.RUnlock()
	if ok && cached.revision == data.Case.Revision {
		return cached.report, nil
	}
	report := readinessForData(&data)
	s.readinessMu.Lock()
	s.readinessCache[caseID] = readinessCacheEntry{revision: data.Case.Revision, report: report}
