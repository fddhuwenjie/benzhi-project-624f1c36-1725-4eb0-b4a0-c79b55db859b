...
func (s *Service) GetCase(ctx context.Context, caseID string) (CaseView, error) {
	...
	cacheKey := struct {
		CaseID   string
		Revision int64
	}{CaseID: caseID, Revision: data.Case.Revision}
	integrity, ok := s.integrityCache.Load(cacheKey)
	if !ok {
		integrity = "valid"
		if err := s.repo.Verify(ctx, caseID); err != nil {
			integrity = err.Error()
		}
		s.integrityCache.Store(cacheKey, integrity)
	}
	...
	return CaseView{
		...
		Events:   data.Events, Manifest: data.Manifest, Integrity: integrity.(string),
	}, nil
}
...
func (s *Service) SegmentPreflight(ctx context.Context, caseID string) (domain.SegmentCoverageReport, error) {
	...
	return domain.InspectSegmentCoverage(data.Segments, data.Case.StandardProfile.ExpectedDurationMillis), nil
}
